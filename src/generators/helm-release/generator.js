...
import { MINIMUM_COUNT } from './models';
...
export async function collector(db, dbExtended) {
    const query = `
    select 
        hrep.helm_repo_url,
        hrep.helm_repo_name,
...
`;
    ...
    return {
        releases: Object.values(releases).map(r => (Object.assign(Object.assign({}, r), { count: count[r.key] }))),
        keys,
        count,
        repos,
        values,
...
export function appDataGenerator(data) {
...
const isObject = (k) => typeof k === 'object' && !Array.isArray(k) && k !== null;
function* visitValues(values) {
...
export function calculateValues(input) {
    const list = input.map(x => [...visitValues(x)]).flat()
        .filter(x => x.name !== "")
        .filter(x => x.type !== "object");
    const names = [...new Set(list.map(x => x.name))];
    const types = list.reduce((map, val) => {
        if (!(val.name in map)) {
    ...
    const idToUrlMap = Object.fromEntries(Object.entries(urlToIdMap).map(([k, v]) => [v, k]));
    ...
    const counts = Object.fromEntries(names
        .map(x => [x, nameUrlMap[x].size]));
    // we want the max count for each prefix
    const countSplitted = Object.entries(counts).map(([name, count]) => {
...
export function pageGenerator({ releases, repos, values, count }, filterRelevant = true) {
    ...
    for (const [key, value] of relevant) {
        ...
        if (fs.existsSync(docPath)) {
            doc = DOMPurify.sanitize(marked.parse(fs.readFileSync(docPath, 'utf-8').replace(/^[\u200B\u200C\u200D\u200E\u200F\uFEFF]/, "")));
        }
        else {
        ...
        pages["/hr/" + key] = {
            ...
            key,
            repos: repos[key],
            values: calculateValues(repos[key].map(r => r.url).map(url => [url, values[url]])),
            doc,
            icon,
            helmRepoName,
            helmRepoURL,
